...
def autocorrelation_comparison(real_data, generated_data, max_lag=20):
    ...
    # Compute autocorrelation for each feature; use NaN-filled arrays and pad ACFs
    real_acf = np.full((real_data.shape[1], max_lag + 1), np.nan, dtype=float)
    gen_acf = np.full((generated_data.shape[1], max_lag + 1), np.nan, dtype=float)

    def _padded_acf(series, max_lag):
        n = len(series)
        if n < 2:
            # not enough observations to compute acf
            return np.full(max_lag + 1, np.nan, dtype=float)
        nlags = min(max_lag, n - 1)
        vals = acf(series, nlags=nlags, fft=True)
        out = np.full(max_lag + 1, np.nan, dtype=float)
    ...
    for i in range(real_data.shape[1]):
        real_acf[i] = _padded_acf(real_data[:, i], max_lag)
        gen_acf[i] = _padded_acf(generated_data[:, i], max_lag)
...
def volatility_clustering(data, window_size=10):
    ...
    vol_acf = np.full((returns.shape[1], window_size + 1), np.nan, dtype=float)

    for i in range(returns.shape[1]):
        vol_acf[i] = _padded_acf(squared_returns[:, i], window_size)
